    ...
    def result_schemas_valid(self, require_entries: bool = True) -> bool:
        ...
        for result_file in self.result_files():
            with open(result_file) as f:
                ...
                schema_dict = load_json_schema(
                    get_schema_repo_path(schema_url))
                jsonschema.validate(instance=envelope["item"],
                                    schema=schema_dict)
                entries_validated += 1
...
def get_schema_repo_path(url: str):
    # e.g. https://raw.githubusercontent.com/nextlinux/vunnel/main/schema/vulnerability/nvd/schema-{version}.json
    relative_path = url.removeprefix(
        "https://raw.githubusercontent.com/nextlinux/vunnel/main/")
...
class Helpers:
    ...
    def provider_workspace_helper(self,
                                  name: str,
                                  create=True) -> WorkspaceHelper:
        root = self.tmpdir
        if create:
