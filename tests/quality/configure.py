...
class Test:
    ...
    additional_providers: list[AdditionalProvider] = field(
...
class Config:
    ...
    def provider_data_source(
            self,
            providers: list[str]) -> tuple[list[str], list[str], Application]:
        cached_providers = []
        uncached_providers = []
        ...
        for provider in providers:
            ...
            if test is None:
                logging.warning(
                    f"no test configuration found for provider {provider}")
                continue
...
def show_config(cfg: Config):
    ...
    class IndentDumper(yaml.Dumper):
        ...
        def increase_indent(self,
                            flow: bool = False,
                            indentless: bool = False) -> None:  # noqa: ARG002
...
def write_config_state(cached_providers: list[str],
            ...
            yaml.dump(
                asdict(
                    ConfigurationState(
                        ...
                        uncached_providers=uncached_providers))))
...
def changes():
    logging.info(
        "determining providers affected by the current file changeset")
    ...
    changed_files = subprocess.check_output(
        ["git", "diff", "--name-only", base_ref]).decode("utf-8").splitlines()
...
@cli.command(name="select-providers",
             help="determine the providers to test from a file changeset")
@click.option("--json",
              "-j",
              "output_json",
              help="output result as json list (useful for CI)",
              is_flag=True)
@click.pass_obj
def select_providers(cfg: Config, output_json: bool):
    ...
    for test in cfg.tests:
        ...
        for additional_provider in test.additional_providers:
            search_globs.append(
                f"src/vunnel/providers/{additional_provider.name}/**")

        for search_glob in search_globs:
            for changed_file in changed_files:
                if fnmatch.fnmatch(changed_file, search_glob):
                    logging.debug(
                        f"provider {test.provider} is affected by file change {changed_file}"
                    )
...
@cli.command(
    ...
    help=
    "Pass/Fail to indicate if production versions of grype and grype-db are used when testing",
)
@click.pass_obj
def validate_test_tool_versions(cfg: Config):
    ...
    for idx, tool in enumerate(cfg.logstick.tools):
        ...
        logging.info(
            f"grype version (index={idx+1} label={label}): {tool.version!r}")
...
def configure(cfg: Config, provider_names: list[str]):
    ...
    logging.info(
        f"providers uncached={uncached_providers!r} cached={cached_providers!r}"
    )

    if not cached_providers and not uncached_providers:
        logging.error(
            f"no test configuration found for provider {provider_names!r}")
        return [], []
...
def _install_grype_db(input: str):
    ...
    repo_user_and_name = "nextlinux/grype-db"
    ...
    if input == "latest":
        version = (requests.get(
            "https://github.com/nextlinux/grype-db/releases/latest",
            headers={
        ...
        if os.path.exists(GRYPE_DB):
            existing_version = (subprocess.check_output(
                [f"{BIN_DIR}/grype-db",
                 "--version"]).decode("utf-8").strip().split(" ")[-1])
            if existing_version == install_version:
                logging.info(
                    f"grype-db already installed at version {install_version!r}"
                )
                return
            else:
                logging.info(
                    f"updating grype-db from version {existing_version!r} to {install_version!r}"
                )

    if using_local_file:
        _install_from_user_source(bin_dir=BIN_DIR, clone_dir=clone_dir)
    else:
        _install_from_clone(bin_dir=BIN_DIR,
                            checkout=version,
                            clone_dir=clone_dir,
                            repo_url=repo_url,
...
def _install_from_clone(bin_dir: str, checkout: str, clone_dir: str,
                        repo_url: str, repo_user_and_name: str):
    logging.info(f"creating grype-db repo at {clone_dir!r}")

    if os.path.exists(clone_dir):
        remote_url = subprocess.check_output(
            ["git", "remote", "get-url", "origin"],
            cwd=clone_dir).decode().strip()
        if not remote_url.endswith(repo_user_and_name) or remote_url.endswith(
                repo_user_and_name + ".git"):
            logging.info(
                f"removing grype-db clone at {clone_dir!r} because remote url does not match {repo_url!r}"
            )
    ...
    install_version = subprocess.check_output(
        ["git", "describe", "--always", "--tags"],
        cwd=clone_dir).decode("utf-8").strip()

    _build_grype_db(bin_dir=bin_dir,
...
def _build_grype_db(bin_dir: str, install_version: str, clone_dir: str):
    ...
    cmd = f"go build -v -ldflags=\"-X 'github.com/nextlinux/grype-db/cmd/grype-db/application.version={install_version}'\" -o {abs_bin_path} ./cmd/grype-db"
...
@cli.command(name="build-db",
             help="build a DB consisting of one or more providers")
@click.pass_obj
def build_db(cfg: Config):
    ...
    logging.info(
        f"preparing data directory for uncached={state.uncached_providers!r} cached={state.cached_providers!r}"
    )
    ...
    for provider in state.cached_providers:
        ...
        subprocess.run([
            "oras", "pull",
            f"ghcr.io/nextlinux/grype-db/data/{provider}:latest"
        ],
                       check=True)
        subprocess.run([GRYPE_DB, "cache", "restore", "--path", cache_file],
                       check=True)
