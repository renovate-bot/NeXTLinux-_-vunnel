...
git clone git@github.com:nextlinux/grype.git
git clone git@github.com:nextlinux/grype-db.git
...
git clone git@github.com:nextlinux/vunnel.git
cd vunnel
...
• Packaging grype-db ...
[0000]  INFO grype-db version: ede464c2def9c085325e18ed319b36424d71180d-adhoc-build
[0000]  INFO packaging DB from="./build" for="https://toolbox-data.nextlinux.io/grype/databases"
...
<!-- asset comment: https://github.com/nextlinux/vunnel/issues/102#issuecomment-1456403838 -->
...
<!-- asset comment: https://github.com/nextlinux/vunnel/issues/102#issuecomment-1456408327 -->
...
a single provider at a time, not orchestrating multiple providers at once. [Grype-db](github.com/nextlinux/grype-db) is the
...
<!-- asset comment: https://github.com/nextlinux/vunnel/issues/102#issuecomment-1456415533 -->
<img src="https://user-images.githubusercontent.com/590471/223165191-8b06b696-f7b5-4a92-912a-c7110c1cd324.svg" width="600" />

For more information about how Grype-DB uses Vunnel see [the Grype-DB documentation](https://github.com/nextlinux/grype-db/blob/main/DEVELOPING.md#architecture).
...
- have a class that implements the [`Provider` interface](https://github.com/nextlinux/vunnel/blob/1285a3be0f24fd6472c1f469dd327541ff1fc01e/src/vunnel/provider.py#L73)
- be centrally registered with a unique name under [`src/vunnel/providers/__init__.py`](https://github.com/nextlinux/vunnel/blob/1285a3be0f24fd6472c1f469dd327541ff1fc01e/src/vunnel/providers/__init__.py)
...
```json
{
  ...
  "schema": {
    "version": "1.0.0",
    "url": "https://raw.githubusercontent.com/nextlinux/vunnel/main/schema/provider-workspace-state/schema-1.0.0.json"
  }
}
```
...
```json
{
  "schema": "https://raw.githubusercontent.com/nextlinux/vunnel/main/schema/vulnerability/os/schema-1.0.0.json",
...
- [Generic OS Vulnerability](https://github.com/nextlinux/vunnel/tree/main/schema/vulnerability/os)
- [GitHub Security Advisories](https://github.com/nextlinux/vunnel/tree/main/schema/vulnerability/github-security-advisory)
- [NVD Vulnerability](https://github.com/nextlinux/vunnel/tree/main/schema/vulnerability/nvd)
...
[Syft](github.com/nextlinux/syft). A prerequisite for adding a new provider is that Syft can catalog the package types that
the provider is feeding vulnerability data for, so [Grype](github.com/nextlinux/grype) can perform the matching from these two sources.
...
All results must conform to a [particular schema](https://github.com/nextlinux/vunnel/tree/main/schema), today there are a few kinds:
...
- Grype does not support the distro type and it needs to be added. See the [grype/distro/types.go](https://github.com/nextlinux/grype/blob/main/grype/distro/type.go) file to add the new distro.
- Grype supports the distro already, but matching is disabled. See the [grype/distro/distro.go](https://github.com/nextlinux/grype/blob/main/grype/distro/distro.go) file to enable the distro explicitly.
- There is a non-standard mapping of distro to namespaces (e.g. redhat and centos map to `rhel`). See the grype db schema namespace index for possible changes: https://github.com/nextlinux/grype/blob/main/grype/db/v5/namespace/index.go .
...
- provider: amazon
  images:
    ...
    - docker.io/nextlinux/test_images:vulnerabilities-amazonlinux-2-5c26ce9@sha256:cf742eca189b02902a0a7926ac3fbb423e799937bf4358b0d2acc6cc36ab82aa
```
...
Later we'll open a PR in the [vulnerability-match-labels repo](github.com/nextlinux/vulnerability-match-labels) to persist these labels.
...
#### **7. Open a [vulnerability-match-labels repo](github.com/nextlinux/vulnerability-match-labels) PR to persist the new labels.**
...
```
# fork the github.com/nextlinux/vulnerability-match-labels repo, but you do not need to clone it...
...
At this point you can open a PR against in the [vulnerability-match-labels repo](github.com/nextlinux/vulnerability-match-labels).
...
The best way is to look for [issues with the `refactor` label](https://github.com/nextlinux/vunnel/issues?q=is%3Aissue+is%3Aopen+label%3Arefactor).
