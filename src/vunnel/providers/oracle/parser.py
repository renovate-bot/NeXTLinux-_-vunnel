...
# regexes
ol_config.tag_pattern = re.compile(
    r"\{http://oval.mitre.org/XMLSchema/.*\}(\w*)")
ol_config.ns_pattern = re.compile(
    r"(\{http://oval.mitre.org/XMLSchema/.*\})\w*")
ol_config.is_installed_pattern = re.compile(
    r"Oracle Linux (\d+).*is installed")
...
class Parser:
    ...
    def __init__(self,
        ...
        self.xml_file_path = os.path.join(workspace.input_path,
                                          self._xml_file_)
    ...
    def _download(self):
        try:
            ...
            r = requests.get(self._url_,
                             stream=True,
                             timeout=self.download_timeout)
            if r.status_code != 200:
                raise Exception(
                    f"GET {self._url_} failed with HTTP error {r.status_code}")
    ...
    def _parse_oval_data(self, path: str, config: dict):
        ...
        # See:https://github.com/nextlinux/nextlinux-engine/issues/1237 for details and links.
...
class KspliceFilterer:
    ...
    def filter(self, vuln_dict: dict) -> dict:  # noqa: A003
        ...
        data model and in Grype. See https://github.com/nextlinux/nextlinux-engine/issues/1237 for FP description
        ...
        for version, vuln in vuln_dict.values():  # noqa
            ...
            if fixes:
                ...
                vuln["Vulnerability"]["FixedIn"] = [
                    fix for fix in fixes
                    if not self._is_ksplice_version(fix.get("Version", ""))
                ]
