...
patch_states = {
    ...
    "needs-triage":
    False,  # Not yet determined if CVE affects package, ignore in nextlinux until determination made
    "ignored":
    False,  # CVE does not affect the package or no updates (e.g. end-of-life) (NOTE: should still report?)
    "not-affected":
    False,  # The package is related to the issue, but not affected by it.
    "needed": True,  # Package is vuln and needs a fix. No version yet.
    "released":
    True,  # The package is affected and a fix has been released with the given version.
    "pending": True,
    ...
    "active":
    True,  # The package is affected, needs fixing and is being worked on.
    "deferred": True
        ...
        for k, v in vars(self).items():
            if not k[0] == "_":
                if type(v) == list or type(v) == set:
                    jsonified[k] = [
                        x.json()
                        if hasattr(x, "json") and callable(x.json) else x
                        for x in v
                    ]
                elif type(v) == dict:
                    jsonified[k] = {
                        x:
                        y.json()
                        if hasattr(y, "json") and callable(y.json) else y
                        for x, y in v.items()
                    }
                else:
                    if hasattr(v, "json"):
...
class CVEFile:
    ...
    def from_dict(d: dict[str, Any]):
        name = d.get("name",
                     d.get("Name", d.get("candidate", d.get("Candidate"))))
...
def check_header(expected: str, lines: list[str]):
    ...
    if not lines[0].strip().startswith(expected + ":"):
        raise ValueError("Expected header {}, found {}".format(
            expected, lines[0]))
    else:
        lines.pop(0)
...
def parse_simple_keyvalue(expected_key: str, lines: list[str]) -> str:
    ...
    if key != expected_key:
        raise ValueError(
            f"Did not find expected key {expected_key}. Found {key}")
...
def check_patch(patch_record: Patch) -> bool:
    ...
    * The release is one we are tracking
    ...
    # Distro filter step
    return check_release(patch_record.distro) and check_state(
        patch_record.status)
...
def check_merge(patch_record: Patch) -> bool:
    ...
    return all(
        patch_dict.get(key, None) and re.match(regex, patch_dict.get(key))
...
def map_parsed(parsed_cve: CVEFile, logger: logging.Logger | None = None):
    ...
    if not (parsed_cve.name):
        logger.error("could not find a Name for parsed cve: {}".format(
            asdict(parsed_cve)))
        return []
    ...
    for p in parsed_cve.patches:
        ...
        if not r:
            ...
            r = Vulnerability()
            try:
                r.Severity = getattr(Severity,
                                     parsed_cve.priority.capitalize())
            except:
        ...
        if check_state(p.status):
            ...
            if p.status == "released":
                # Can do version format check here, but requires code from nextlinux-engine
                # nextlinux_engine.services.policy_engine.engine.util.deb.DpkgVersion.from_string(p.get('status'))
                pkg.Version = p.version
                if pkg.Version is None:
                    logger.debug(
                        'found CVE {} in ubuntu version {} with "released" status for pkg {} but no version for release. Released patches should have version info, but missing in source data. Marking package as not vulnerable'
                        .format(r.Name, r.NamespaceName, pkg.Name))
                    continue
...
def filter_resolved_patches(
        cve: CVEFile, dpt_list: list[DistroPkg]) -> dict[DistroPkg, Patch]:
    """
    ...
    for dpt in dpt_list:
        matched_p = next(
            (p for p in cve.patches if dpt.distro == p.distro
             and dpt.pkg == p.package and not check_merge(p)),
            None,
        )
        if matched_p:
...
def filter_merged_patches(cve: CVEFile,
                          dpt_list: list[DistroPkg]) -> dict[DistroPkg, Patch]:
    """
    ...
    for dpt in dpt_list:
        matched_p = next(
            (p for p in cve.patches
             if dpt.distro == p.distro and dpt.pkg == p.package),
            None,
        )
        if matched_p:
            filtered_map[dpt] = matched_p
...
class Parser:
    ...
    data_filename_regex = re.compile(
        r"(active|retired|ignored)/CVE-[0-9]{4}-[0-9]+")
    ...
    def __init__(
        ...
        self.vc_workspace = os.path.join(workspace.input_path,
                                         self._vc_working_dir)
        # TODO: tech debt: this should use the results workspace with the correct schema-aware envelope
        self.norm_workspace = os.path.join(workspace.input_path,
                                           self._normalized_cve_dir)
        if not logger:
        ...
        self.git_wrapper = GitWrapper(source=self.git_url,
                                      branch=self.git_branch,
                                      checkout_dest=self.vc_workspace,
                                      logger=logger)
    ...
    def fetch(self, skip_if_exists=False):
        ...
        self._process_data(self.vc_workspace,
                           from_rev=last_rev,
                           to_rev=current_rev)
        ...
        self.logger.info(
            "begin loading processed CVE content and transforming into vulnerabilities"
        )
        for merged_cve in self._merged_cve_iterator():
            yield from map_parsed(merged_cve, self.logger)
        self.logger.info(
            "finish loading processed CVE content and transforming into vulnerabilities"
        )
    ...
    def _process_data(self,
        ...
        self.logger.info(
            f"processing data from git repository: {vc_dir}, from revision: {from_rev}, to revision: {to_rev}"
        )
        ...
        if from_rev and to_rev and from_rev != to_rev:
            self.logger.debug(
                "fetching changes to CVEs between revisions {} and {}".format(
                    from_rev, to_rev))
            modified, removed = self.git_wrapper.get_merged_change_set(
            ...
            self.logger.info(
                "detected {} CVE updates (add/modify/rename)".format(
                    len(updated_paths)))
            self.logger.info("detected {} CVE deletions".format(
                len(deleted_ids)))
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for d in [self._active_cve_dir, self._retired_cve_dir]:
                ...
                for cve_id in sorted(
                        filter(lambda x: _cve_filename_regex.match(x),
                               os.listdir(cve_dir))):
                    f = os.path.join(cve_dir, cve_id)
                    cve_rel_path = "/".join([d, cve_id])

                    future = executor.submit(worker, self._process_cve, cve_id,
                                             cve_rel_path, f, to_rev,
                                             updated_paths)
            ...
            if len(done) > 0 and len(done) != len(futures):
                ...
                if proc_exception:
                    self.logger.error(
                        f"one task failed with: {proc_exception} (shutting down)"
                    )
        ...
        for cve_id in deleted_ids:
            self.logger.debug(
                "{} is no longer relevant, deleting merged CVE state if any".
                format(cve_id))
            self._delete_merged_cve(cve_id)
        self.logger.info("finish processing deletes")

    def _process_cve(self, cve_id: str, cve_rel_path: str, f: str, to_rev: str,
    ...
    def _load_last_processed_rev(self):
        last_processed_rev_path = os.path.join(
            self.norm_workspace, self._last_processed_rev_file_git)
        ...
        if os.path.exists(last_processed_rev_path):
            ...
            last_processed_rev_path = os.path.join(
                self.norm_workspace, self._last_processed_rev_file)
            if os.path.exists(last_processed_rev_path):
    ...
    def _save_merged_cve(self, cve_id: str, merged_cve: CVEFile):
        ...
        with open(filepath, "wb") as f:
            self.logger.trace(
                f"writing record to {filepath!r}")  # type: ignore
    ...
    def _merged_cve_iterator(self) -> Generator[CVEFile, None, None]:
        for cve_id in filter(lambda x: _cve_filename_regex.match(x),
                             os.listdir(self.norm_workspace)):
            with open(os.path.join(self.norm_workspace, cve_id)) as fp:
                cve = json.load(fp)
    ...
    def _reprocess_merged_cve(self, cve_id: str, cve_rel_path: str):
        ...
        merged_patches, ignored_patches, to_be_merged_map = self._categorize_patches(
            saved_state.ignored_patches)
        ...
        if merged_patches or to_be_merged_map:
            self.logger.debug(
                "found unresolved patches in previously merged state, could be a new distro"
            )
            ...
            if to_be_merged_map:
                if self.enable_rev_history:
                    self.logger.debug(
                        "attempting to resolve patches using revision history for {}"
                        .format(cve_rel_path))
                    (
                        resolved_patches,
                        pending_dpt_list,
                    ...
                    if pending_dpt_list:
                        self.logger.debug(
                            "exhausted all revisions for {} but could not resolve patches: {}"
                            .format(
                                cve_rel_path,
                                [
                                    to_be_merged_map[x]
                                    for x in pending_dpt_list
                                ],
                            ))
                    ...
                    self.logger.debug(
                        "revision history processing is disabled. Merging unresolved patches as they are"
                    )
    ...
    @staticmethod
    def _categorize_patches(
        patch_list: list[Patch]
    ) -> tuple[list[Patch], list[Patch], dict[DistroPkg, Patch]]:
        """
    ...
    def _merge_cve(self, cve_id: str, cve_rel_path: str, cve_abs_path: str,
                   repo_current_rev: str):
        """
        ...
        if to_be_merged_map:
            ...
            if self.enable_rev_history:
                ...
                if saved_cve:
                    self.logger.debug(
                        "revision history processing is disabled. Resolving patches using saved cve state"
                    )
                    rev_matched_map = filter_merged_patches(
                        saved_cve, list(to_be_merged_map.keys()))
                    # merge resolved and unresolved patches
                    merged_patches.extend(list(rev_matched_map.values()))
                    merged_patches.extend([
                        to_be_merged_map[x] for x in to_be_merged_map
                        if x not in rev_matched_map
                    ])
    ...
    def _resolve_patches_using_history(
        ...
        t = time.time()
        self.logger.debug(
            f"processing CVE revision history for: {cve_rel_path}")
        ...
        pending_dpt_list: list[DistroPkg] = copy.deepcopy(
        ...
        since_revs = self.git_wrapper.get_revision_history(
            cve_id, cve_rel_path, saved_cve_last_processed_rev)

        # make note of the first revision, its the latest revision and one we already processed
        cve_latest_rev = since_revs[
            0].sha if since_revs and since_revs[0] else None
        ...
        if since_revs and len(since_revs) > 1:
            ...
            for rev in since_revs[1:]:
                ...
                rev_matched_map = filter_resolved_patches(
                    rev_cve, pending_dpt_list)
                resolved_patches.extend(list(rev_matched_map.values()))
                pending_dpt_list = [
                    x for x in pending_dpt_list if x not in rev_matched_map
                ]
                ...
                for rev_p in rev_matched_map.values():
                    ...
                    if patch_status not in metrics["previous_state_counter"]:
                        metrics["previous_state_counter"][patch_status] = {
                            rev_p_ns: 0
                        }
                    if patch_distro not in metrics["previous_state_counter"][
                            patch_status]:
                        metrics["previous_state_counter"][patch_status][
                            rev_p_ns] = 0
        ...
        if pending_dpt_list and saved_state:
            self.logger.debug(
                "resolving patches using saved cve state before processing any more revisions"
            )
            rev_matched_map = filter_merged_patches(saved_state,
                                                    pending_dpt_list)
            resolved_patches.extend(list(rev_matched_map.values()))
            pending_dpt_list = [
                x for x in pending_dpt_list if x not in rev_matched_map
            ...
            if pending_dpt_list:
                ...
                self.logger.debug(
                    "unresolved patches found even after merge with saved state, walking entire revision history"
                )
                all_revs = self.git_wrapper.get_revision_history(
                    cve_id, cve_rel_path, None)
                before_revs = all_revs[len(since_revs
                                           ):] if since_revs else all_revs
                ...
                for rev in before_revs:
                    ...
                    for rev_p in rev_matched_map.values():
                        rev_p_ns = map_namespace(rev_p.distro)
                        if rev_p.status not in metrics[
                                "previous_state_counter"]:
                            metrics["previous_state_counter"][rev_p.status] = {
                                rev_p_ns: 0
                            }
                        if rev_p.distro not in metrics[
                                "previous_state_counter"][rev_p.status]:
                            metrics["previous_state_counter"][
                                rev_p.status][rev_p_ns] = 0

                        metrics["previous_state_counter"][
                            rev_p.status][rev_p_ns] += 1
                ...
                self.logger.debug(
                    "Merge with saved state resolved all relevant patches")

        metrics["time_elapsed"] = int((time.time() - t) * 1000) / float(1000)
        self.logger.trace(
            "metrics from processing revision history for {}: {}".format(
                cve_rel_path, metrics))
