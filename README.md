...
docker run \
  ...
  -v $(pwd)/.vunnel.yaml:/.vunnel.yaml \
    ghcr.io/nextlinux/vunnel:latest  \
      run nvd
```
...
See [the vunnel package](https://github.com/nextlinux/vunnel/pkgs/container/vunnel) for a full listing of available tags.
...
_Note: to get more verbose output, use `-v`, `-vv`, or `-vvv` (e.g. `vunnel -vv run wolfi`)_
...
Yes you can! See [the provider docs](https://github.com/nextlinux/vunnel/blob/main/DEVELOPING.md#adding-a-new-provider) for more information.
